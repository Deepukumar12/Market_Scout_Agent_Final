...
const TargetUniversePage = () => {
  ...
  const { competitors, selectedCompetitorId } = useCompetitorStore();
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const dataSignals = await getIntelligenceStream(50, activeCompetitorName);
  ...
  }, [fetchData, activeCompetitorName]);
      ...
      <div className="min-h-screen bg-[#FBFBFE] dark:bg-[#050505] p-6 lg:p-10 flex items-center justify-center">
        <div className="max-w-md w-full glass-card rounded-[32px] p-8 border border-[#E5E5EA] dark:border-white/10 bg-white/80 dark:bg-[#0A0A0C]/80 shadow-2xl text-center space-y-6">
          ...
          <p className="text-sm text-[#86868B] leading-relaxed">
            Initialize your surveillance network by adding a competitor.
          </p>
          <Button 
    ...
    return (
      <div className="min-h-screen bg-[#FBFBFE] dark:bg-[#050505] p-6 lg:p-10 flex items-center justify-center">
        <div className="max-w-md w-full glass-card rounded-[32px] p-8 border border-[#E5E5EA] dark:border-white/10 bg-white/80 dark:bg-[#0A0A0C]/80 shadow-2xl text-center space-y-4">
          ...
          <p className="text-sm text-[#86868B] leading-relaxed">
            Please select a surveillance target from the top navbar to display competitor signals.
          </p>
        </div>
      </div>
    );
        ...
        <div className="lg:col-span-2 space-y-10">
          <div className="bg-white/70 dark:bg-[#1D1D1F]/70 border border-[#E5E5EA] dark:border-white/10 rounded-[48px] overflow-hidden backdrop-blur-3xl shadow-apple-large transition-all duration-500">
            ...
            <div className="divide-y divide-[#E5E5EA] dark:divide-white/5 max-h-[800px] overflow-y-auto custom-scrollbar">
              {signals.length === 0 ? (
                <div className="p-16 text-center text-[#86868B] dark:text-[#A1A1A6] font-black uppercase tracking-widest text-xs italic">
                  No updates found within the last 7 days.
                </div>
              ) : (
                      ...
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-[10px] text-[#86868B] dark:text-[#A1A1A6] font-black uppercase tracking-widest mt-1">
                            ...
                            <span className="w-1.5 h-1.5 bg-[#E5E5EA] dark:bg-white/20 rounded-full" />
                            <span className="font-mono">{formatToIST(signal.timestamp)}</span>
                          </div>
                        </div>
                      </div>
